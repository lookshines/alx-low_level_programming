#include "main.h"

char *create_buffer(char *file);
void close_file(int fd);

/**
 * create_buffer - allocate 1024 bytes for the buffer.
 * @file: The name of the file buffer is stored
 *
 * Return: A pointer to the newly-allocated buffer.
 */

char *create_buffer(char *file)
{
	char *buffer;

	buffer = malloc(sizeof(char) * 1024);

	if (buffer == NULL)
	{
		dprintf(STDERR_FILENO, "Error: Can't write to %s\n", file);
		exit(99);
	}

	return (buffer);
}

/**
 * close_file - Close file discriptors.
 * @fd: The file descriptor to be close.
 */
void close_file(int fd)
{
	int c;

	c = close(fd);

	if (c == -1)
	{
		dprintf(STDERR_FILENO, "Error: Can't close fd %d\n", fd);
		exit(100);
	}
}

/**
 * main - copies the content of a file to another file.
 * @argc: The number of arguments provided
 * @argv: An array of points to the arguments.
 *
 * Return: 0 on Success
 *
 * Description: if you can not create file exit with code 99
 *		if write to file_to fails, exit with code 99
 *		if you can not close a file descriptor , exit with code 100
 */
int main(int argc, char *argv[])
{
	int from, to, rd, wr;
	char *buffer;

	/*Check if the correct number of argument are provided*/
	if (argc != 3)
	{
		dprintf(STDERR_FILENO, "Usage: cp file_from file_to\n");
		exit(97);
	}

	/*Create the require buffer*/
	buffer = create_buffer(argv[2]);

	/*Open file to copy from*/
	from = open(argv[1], O_RDONLY);

	/*Read from the file to copy from*/
	rd = read(from, buffer, 1024);

	/*Open the second file*/
	to = open(argv[2], O_CREAT | O_WRONLY | O_TRUNC, 0664);

	do {
		if (from == -1 || rd == -1)
		{
			dprintf(STDERR_FILENO, "Error: Can't read from file %s\n", argv[1]);
			free(buffer);
			exit(98);
		}
		/*Copy into the second file*/
		wr = write(to, buffer, rd);

		if (to == -1 || wr == -1)
		{
			dprintf(STDERR_FILENO, "Error: Can't write to %s\n", argv[2]);
			free(buffer);
			exit(99);
		}

		rd = read(from, buffer, 1024);
		to = open(argv[2], O_WRONLY | O_APPEND);

	} while (rd > 0);

	free(buffer);
	close_file(from);
	close_file(to);

	return (0);
}
